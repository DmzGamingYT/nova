...
const SKILLS = {
  ...
  async battery() {
    if (process.platform !== 'darwin') return { ok: false, error: 'batterie disponible seulement sur macOS' };
  ...
  uptime() {
    ...
    return { ok: true, output: 'le Mac est allumé depuis ' + (bits.join(' et ') || 'moins d\u2019une minute') };
  },
  async clipboard() {
    if (process.platform !== 'darwin') return { ok: false, error: 'presse-papiers disponible seulement sur macOS' };
